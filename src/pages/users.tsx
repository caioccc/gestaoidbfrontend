...
export default function UsersPage() {
  ...
  const churchId = user?.church?.id ?? null;
  ...
  const handleRoleChange = async (membershipId: number, role: Role) => {
    if (!churchId || !canManageUsers || membershipId === user?.id) return;
  ...
  const handleRemove = async () => {
    if (!churchId || !canManageUsers || !toRemove) return;
    setRemoving(true);
    try {
    ...
    <AuthGuard roles={['PASTOR', 'SECRETARIA']}>
      <Layout>
        <PageHeader
          ...
          description={`${t.usersPage.subtitle} — ${user?.church?.name}`}
        >
          <Group gap="xs">
            <Button
              variant="default"
